...
if (request_count($session_user_id) > 1) {
    ...
    $requests = get_friends($session_user_id);
    foreach ($requests as $friend) {
        $request_data   = user_data($friend['friend_id'], 'username', 'first_name', 'last_name', 'profile');
        echo "<div style='text-align: center' class='span2'><img class='img-polaroid' style='max-width:100%;' src=" . $request_data['profile'] . " /><a href=" . $base_url . "profile/" . $request_data["username"] . " ><h5>" . $request_data['first_name'] . " " . $request_data['last_name'] . "</h5></a></div>";
    }
} ?>
    </div></div>
